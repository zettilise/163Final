...
def make_mono(poly):
    ...
    for v in stopping:
        """
        # Based on vertex types, as defined by David Mount's comp geo notes
        if v.y < v.v_prev.y:
            if v.x < v.v_prev.x:
                # Start vertex
                v.type = "start"
                ...
                if v.x < v.v_next.x:
                    ...
                    if edge_prev.helper is not None and edge_prev.helper.type == "merge":
                        graph.add_edge(v.name, edge_prev.helper.name)
